...
import { resolveFormat, loadConfig, type Job, type Config } from '../config';
...
describe('loadConfig', () => {
	...
	it('throws when explicit config file does not exist', () => {
		expect(() => loadConfig(path.join(tmpDir, 'nonexistent.json'), true)).toThrow('Config file not found');
	});
