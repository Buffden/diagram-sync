...
Same as above but commits the generated SVGs back to the repo via a pull request. This works with branch protection rules on `main`.

**Flow:**
1. You push a `.puml` file change to any branch
2. Workflow triggers, generates SVGs
3. SVGs are committed to a `chore/auto-export-diagrams` branch
4. A PR is automatically opened targeting `main`
5. You review and merge the PR

Requires a **Personal Access Token (PAT)** with `contents: write` and `pull-requests: write` permissions added as a repository secret named `PAT_TOKEN`. See [how to create a PAT](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens).
...
jobs:
  generate:
    ...
    steps:
      ...
      - name: Commit generated SVGs via PR
        run: |
          ...
          if git diff --staged --quiet; then
            ...
            git checkout -b chore/auto-export-diagrams
            git commit -m "chore: auto-export diagrams [skip ci]"
            git push origin chore/auto-export-diagrams --force
            gh pr create --title "chore: auto-export diagrams" \
              --body "Auto-generated by diagram-sync" \
              --base main \
              --head chore/auto-export-diagrams || echo "PR already exists"
          fi
        env:
          GH_TOKEN: ${{ secrets.PAT_TOKEN }}
```

> **No branch protection?** Remove the `token` from the checkout step, replace `PAT_TOKEN` with `GITHUB_TOKEN`, remove the `gh pr create` step, and push directly to `main`. Also enable **Read and write permissions** under **GitHub → Settings → Actions → General → Workflow permissions**.
